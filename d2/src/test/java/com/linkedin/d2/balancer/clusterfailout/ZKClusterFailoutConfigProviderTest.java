...
public class ZKClusterFailoutConfigProviderTest
{
  ...
  private ZKClusterFailoutConfigProvider<TestingClusterFailoutConfig> _clusterFailoutConfigProvider;
  ...
  public void testNewCluster()
  {
    ...
    assertFalse(_clusterFailoutConfigProvider.getFailoutConfig(CLUSTER_NAME).isPresent());
  }
  ...
  public void testModifyClusterConfig()
  {
    ...
    Optional<TestingClusterFailoutConfig> config = _clusterFailoutConfigProvider.getFailoutConfig(CLUSTER_NAME);
    assertTrue(config.isPresent());
    assertFalse(config.get().isFailedOut());
    assertTrue(config.get().getPeerClusters().isEmpty());
    ...
    assertTrue(config.isPresent());
    assertTrue(config.get().isFailedOut());
    assertEquals(config.get().getPeerClusters(), Collections.singletonList(PEER_CLUSTER_NAME1));
    ...
    assertTrue(config.isPresent());
    assertTrue(config.get().isFailedOut());
    assertEquals(config.get().getPeerClusters(), new HashSet<>(Arrays.asList(PEER_CLUSTER_NAME1, PEER_CLUSTER_NAME2)));
  ...
  private static class TestingZKClusterFailoutConfigProvider extends ZKClusterFailoutConfigProvider<TestingClusterFailoutConfig>
  {
    ...
    public TestingClusterFailoutConfig createFailoutConfig(@Nullable ClusterFailoutProperties clusterFailoutProperties)
    {
      ...
      Set<String> peerClusters = clusterFailoutProperties.getClusterFailoutRedirectConfigs().stream().map(ClusterFailoutRedirectConfig::getFabricUrn)
  ...
  private ClusterStoreProperties createClusterStoreProperties(boolean hasClusterFailoutConfig, boolean isFailedOut, Set<String> peerClusters)
  {
    ...
    if (hasClusterFailoutConfig)
    {
      ...
      peerClusters.forEach(cluster -> redirectConfigs.add(Collections.singletonMap("fabric", cluster)));
      if (!isFailedOut)
      {
